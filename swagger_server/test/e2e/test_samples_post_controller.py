...
def test_sample_is_not_created_if_experiment_id_existed(old, new, create_sample, sample_graph):
    try:
...
def test_sample_is_created_if_experiment_id_not_existed(sample, create_sample, sample_graph):
    try:
...
@given(sample=samples(), neighbour_list=lists(neighbours(), unique_by=lambda x: x.experiment_id, min_size=1))
def test_relationships_with_existing_leaves_and_neighbours_are_created(sample, neighbour_list, create_sample, create_leaf, sample_graph):
    assume(sample.nearest_leaf_node)
    assume(sample.experiment_id not in [x.experiment_id for x in neighbour_list])

    sample.nearest_neighbours = neighbour_list
